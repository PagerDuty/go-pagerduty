...
func WithScopedOAuth(clientId string, clientSecret string, scopes []string) ClientOptions {
	ts := baseTokenSource(context.Background(), clientId, clientSecret, scopes)

	return func(c *Client) {
		c.authType = scopedOAuthToken
		c.tokenSource = ts
	}
}

func WithScopedOAuthTokenSource(tokenSource oauth2.TokenSource) ClientOptions {
	return func(c *Client) {
		c.authType = scopedOAuthToken
